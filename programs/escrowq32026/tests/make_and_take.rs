...
fn test_make_and_take() {
    ...
    let (mut program, maker_key, taker_key) = setup_test();

    // Get the maker's public key from the payer keypair
    let maker = maker_key.pubkey();

    let taker = taker_key.pubkey();
    ...
    let mint_a = CreateMint::new(&mut program, &taker_key)
        .decimals(6)
        .authority(&maker)
        .send()
        .unwrap();
    ...
    let mint_b = CreateMint::new(&mut program, &taker_key)
        .decimals(6)
        .authority(&taker)
        .send()
        .unwrap();
    ...
    let maker_ata_a = CreateAssociatedTokenAccount::new(&mut program, &taker_key, &mint_a)
        .owner(&maker)
        .send()
        .unwrap();
    msg!("Maker ATA A: {}\n", maker_ata_a);

    let maker_ata_b = CreateAssociatedTokenAccount::new(&mut program, &taker_key, &mint_b)
        .owner(&maker)
        .send()
        .unwrap();
    msg!("Maker ATA B: {}\n", maker_ata_b);

    let taker_ata_a = CreateAssociatedTokenAccount::new(&mut program, &taker_key, &mint_a)
        .owner(&taker)
        .send()
        .unwrap();
    msg!("Taker ATA A: {}\n", taker_ata_a);

    let taker_ata_b = CreateAssociatedTokenAccount::new(&mut program, &taker_key, &mint_b)
        .owner(&taker)
        .send()
        .unwrap();
    msg!("Taker ATA B: {}\n", taker_ata_b);
    ...
    let message = Message::new(&[make_ix], Some(&maker_key.pubkey()));
    let recent_blockhash = program.latest_blockhash();

    let transaction = Transaction::new(&[&maker_key], message, recent_blockhash);
    ...
    let message = Message::new(&[take_ix], Some(&taker_key.pubkey()));
    let recent_blockhash = program.latest_blockhash();

    let transaction = Transaction::new(&[&taker_key], message, recent_blockhash);
